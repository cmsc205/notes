...
In the `base` package (loaded by default), there are ways to manipulate character strings, most of which we won't discuss, but we do need a discuss a few:
...
`paste` allows us to easily create strings.
...
days <- "MWF"
class <- "CMSC 205"
paste(class, "meets from", begin_time, "to", end_time, days, "in", building, room)
```
