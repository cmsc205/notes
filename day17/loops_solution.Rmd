...
```{r}
colnames(movies)[5:9] <- c("Domestic_gross", "Domestic_pct", "Overseas_gross", "Overseas_pct", "Year")
head(movies)

untidy_movies <- movies
```

Now we need to remove non-numeric characters from 6 columns

```{r message = FALSE}
library(tidyverse)
movies <- movies %>%
  mutate(Domestic_gross = parse_number(Domestic_gross),
         Worldwide = parse_number(Worldwide),
         Domestic_pct = parse_number(Domestic_pct),
         Overseas_gross = parse_number(Overseas_gross),
         Overseas_pct = parse_number(Overseas_pct),
...
for(col in problem_cols) {
  untidy_movies[[col]] <- parse_number(untidy_movies[[col]])
}
tidy_movies <- untidy_movies
```
