...
p <- p + labs(title = "Average Next Cost per Year at U.S. Colleges")
p
```
...
babynames$name <- reorder(babynames$name, 
...
p2 <- babynames %>%
  ...
  geom_bar(stat = "identity") + 
  coord_flip() +
  labs(title = "Top 10 Baby Names in 2014",
       x = NULL,
       y = "Count")
...
p2 + scale_fill_manual(values = c("turquoise", "gold"))
```
