...
for(b in 1:B) {
  boot_sample <- sample(our_sample$TotalCompensation, size = n, 
                        replace = TRUE)
...
```{r}
boot_df <- data.frame(means = medians)
...
B <- 5000 
n <- nrow(our_sample)
intervals <- data.frame(rep = integer(100), lower = numeric(100), upper = numeric(100))
...
for(i in 1:100){
  ...
  medians <- numeric(length = B)
  for(b in 1:B) {
    boot_sample <- sample(in_hand$TotalCompensation, size = n, 
                          replace = TRUE)
    medians[b] <- median(boot_sample)
  }
